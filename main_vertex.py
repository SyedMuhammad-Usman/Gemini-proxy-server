...
usage_count = 0
VERTEX_MODEL = os.getenv("VERTEX_MODEL", "gemini-2.5-flash-lite-preview-06-17")
...
def get_vertex_model() -> GenerativeModel:
    ...
    Required env vars:
        VERTEX_PROJECT   – your GCP project ID  (e.g. "my-project-123")
        VERTEX_LOCATION  – GCP region           (e.g. "us-central1")
        VERTEX_MODEL     – model name           (e.g. "gemini-2.5-flash-lite-preview-06-17")
    ...
    if not project:
        raise HTTPException(
            status_code=500,
            detail="VERTEX_PROJECT env var is missing",
        )
